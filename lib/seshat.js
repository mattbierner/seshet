...
define([], function() {
    ...
    (Node.update = function(root, compare, pos, id, val) {
        ...
        return rot(((diff < 0) ? Node.setLeft(root, Node.update(root.left, compare, pos, id, val)) : Node.setRight(root, Node.update(root.right, compare, pos, id, val))));
    }
    );
    (Node.rebalance = function(root) {
        return ((Math.abs(df(root)) <= 1) ? root : rot(Node.setChildren(root, Node.rebalance(root.left), Node.rebalance(root.right))));
    }
    );
